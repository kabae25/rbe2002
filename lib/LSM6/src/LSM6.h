...
class LSM6
{
  public:
    ...
    vector<int16_t> h; // heading vector
    vector<int16_t> prev_h; // previous heading vector

    const float SIGMA = 0.99; // for updating bias was 0.95
...
public:
    ...
    void updateGyroBias(void) {
      // take the moving average of the gyro bias with current gyro readings
      gyroBias.x = SIGMA * gyroBias.x + (1-SIGMA)*g.x;
      gyroBias.y = SIGMA * gyroBias.y + (1-SIGMA)*g.y; // we can calibrate the bias here even though we will constantly update it in the future
